...
import type {
  PristupRezervacijiZahtev,
} from '../models/PristupRezervaciji'

import type {
  AutorizacijaRezervacijeZahtev,
  DodajStavkuRezervacijeZahtev,
  PorukaOdgovor,
} from '../models/UpravljanjeRezervacijom'
...
export function getStatusRezervacije(
  idZahteva: string,
) {
...
export function pristupiRezervaciji(
  zahtev: PristupRezervacijiZahtev,
) {
  return apiRequest<RezervacijaDetalji>(
    '/api/rezervacije/pristup',
    {
      method: 'POST',
      body: JSON.stringify(zahtev),
    },
...
export function dodajStavkuRezervaciji(
  rezervacijaId: number,
  zahtev: DodajStavkuRezervacijeZahtev,
) {
  return apiRequest<PorukaOdgovor>(
...
export function ukloniStavkuIzRezervacije(
  rezervacijaId: number,
  stavkaId: number,
